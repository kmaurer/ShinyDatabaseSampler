...
\citet{Doi2016web} used \texttt{shiny} to create a collection of apps for their particular course needs. They fall largely in the footsteps of previous web application for statistics education, such as the collection of Javascript Applets by \citet{chance2006using} and Statkey by \citet{morgan2014statkey}, but are built in the \texttt{R} environment; a setting where more statistician feel comfortable to work and create cumstomized course content. Indeed, many others in the statistics education community have been creating custom apps for -- and in some cases with -- their students (\citealt{Adrian2018}; \citealt{hare2016}; \citealt{BOAST}). 
...
The Shiny Database Sampler was developed for applications in statistics education and was implemented in the initial stages of development within the curriculum of an introductory statistics course. An alpha version of the software was used within a section of an introductory statistics course at a large public midwestern univer for a group lab assignment and a course project. The course was an algebra-based introductory course for non-STEM majors. Later, a beta version was extended to be used in a lab assignment for six more sections of the same course. These two applications demonstrate possible uses of the Shiny Database Sampler. 
